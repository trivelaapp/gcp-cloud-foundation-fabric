...
module "project" {
  ...
  policy_list = {
    "constraints/compute.restrictLoadBalancerCreationForTypes" = {
      ...
      status              = true
      values              = ["INTERNAL_TCP_UDP", "INTERNAL_HTTP_HTTPS", "EXTERNAL_HTTP_HTTPS"]
    },
...
resource "google_compute_security_policy" "policy" {
  ...
  rule {
    ...
    match {
      ...
      config {
        src_ip_ranges = ["${module.vm_siege.external_ip}/32"]
      }
    }
